...
{  
   ...
   /** Crates an ExtendedInviteDialog and initializes the dialog state information. */
   public ExtendedInviteDialog(SipProvider provider, ExtendedInviteDialogListener d_listener)
   {  super(provider,d_listener);
      dialog_listener=d_listener;
   ...
   {  printLog("inside acceptRefer(refer)",LogLevel.MEDIUM);
      Message resp=MessageFactory.createResponse(req,202,"Accepted",null,null);
      respond(resp);
   } 
   ...
   {  printLog("inside refuseRefer(refer)",LogLevel.MEDIUM);
      Message resp=MessageFactory.createResponse(req,603,"Decline",null,null);
      respond(resp);
   } 
   ...
   {  printLog("Message received: "+msg.getFirstLine().substring(0,msg.toString().indexOf('\r')),LogLevel.LOW);
      ...
      {  TransactionServer t=new TransactionServer(sip_provider,msg,this);
         ...
         if (msg.isNotify())
         {  Message resp=MessageFactory.createResponse(msg,200,"OK",null,null);
            respond(resp);
   ...
   {  printLog("inside onCltFailureResponse("+t.getTransactionId()+",msg)",LogLevel.LOW);
      ...
      if (method.equals(SipMethods.INVITE) || method.equals(SipMethods.CANCEL) || method.equals(SipMethods.BYE))
      {  super.onCltFailureResponse(t,msg);
      }
      else
   ...
   {  printLog("inside onCltSuccessResponse("+t.getTransactionId()+",msg)",LogLevel.LOW);
      ...
      if (method.equals(SipMethods.INVITE) || method.equals(SipMethods.CANCEL) || method.equals(SipMethods.BYE))
      {  super.onCltSuccessResponse(t,msg);
      }
      else
      if (t.getTransactionMethod().equals(SipMethods.REFER))
      {  transactions.remove(t.getTransactionId());
   ...
   {  super.printLog("SIPX: "+str,level+SipStack.LOG_LEVEL_DIALOG);  
   }
