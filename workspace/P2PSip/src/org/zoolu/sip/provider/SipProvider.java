...
public class SipProvider implements Configurable, TransportListener,
	...
	/**
	 * Outbound proxy. Use 'NONE' for not using an outbound proxy (or let it
	 * undefined).
	 */
	...
	public SipProvider(String via_addr, int port) { // init(via_addr,port,null,0,null,null,0);
	...
	public SipProvider(String via_addr, int port, String[] protocols,
			String ifaddr) { // init(via_addr,port,protocols,0,null,null,0);
	 ...
	 */
	/*
	 * public SipProvider(String via_addr, int port, String[] protocols, int
	 * nmax_connections) {
	 * init(via_addr,port,protocols,nmax_connections,null,null,0); initlog();
	 * startTrasport(); }
	 */

	/**
	 * Creates a new SipProvider. Costructs the SipProvider, initializing the
	 * SipProviderListeners, the transport protocols, the outbound proxy, and
	 * other attributes.
	 */
	/*
	 * public SipProvider(String via_addr, int port, String[] protocols, int
	 * nmax_connections, String host_ifaddr, String outbound_addr, int
	 * outbound_port) {
	 * init(via_addr,port,protocols,nmax_connections,host_ifaddr
	 * ,outbound_addr,outbound_port); initlog(); startTrasport(); }
	 */
	 ...
	 */
	// private void init(String viaddr, int port, String[] protocols, int
	// nmaxconns, String ifaddr, String outboundaddr, int outboundport)
	private void init(String viaddr, int port, String[] protocols, String ifaddr) {
		...
		if (outbound_addr != null
				&& (outbound_addr.equalsIgnoreCase(Configure.NONE) || outbound_addr
						.equalsIgnoreCase("NO-OUTBOUND")))
			outbound_addr = null;
		if (outbound_port == 0)
	...
	public void parseLine(String line) {
		...
		if (attribute.equals("use_outbound"))
			System.err
					.println("WARNING: parameter 'use_outbound' is no more supported; use 'outbound_addr' for setting an outbound proxy or let it undefined.");
	}
	...
	private ConnectionIdentifier sendMessage(Message msg, String proto,
		...
		if (foot_print == null)
			foot_print = "NOT a SIP message\r\n";
		printLog("Message sent to " + proto + ":" + dest_addr + ":" + dest_port
				+ " (" + msg.getLength() + " bytes): " + foot_print,
				LogLevel.HIGH);
		return conn_id;
	}
	 ...
	 *         (e.g. UDP)
	 */
	public ConnectionIdentifier sendMessage(Message msg) { // logs
		String foot_print = msg.getFirstLine();
		if (foot_print == null)
			foot_print = "NOT a SIP message\r\n";
		printContLog("Sending message (" + msg.getLength() + " bytes): "
				+ foot_print, LogLevel.HIGH);
	...
	public ConnectionIdentifier sendMessage(Message msg,
			ConnectionIdentifier conn_id) { // logs
		String foot_print = msg.getFirstLine();
		if (foot_print == null)
			foot_print = "NOT a SIP message\r\n";
		printContLog("Sending message through conn " + conn_id + ": "
				+ foot_print, LogLevel.HIGH);
		printLog("message:\r\n" + msg.toString(), LogLevel.LOWER);
		...
		if (conn_id != null && connections.containsKey(conn_id)) { // connection
			...
			try {
				...
				printMessageLog(proto, dest_addr, dest_port, msg.getLength(),
						msg, "sent");
				printLog("Message sent to " + proto + ":" + dest_addr + ":"
						+ dest_port + " (" + msg.getLength() + " bytes): "
						+ foot_print, LogLevel.HIGH);
				return conn_id;
			} catch (Exception e) {
				printException(e, LogLevel.HIGH);
	...
	protected void processReceivedMessage(Message msg) {
		try { // logs
			...
			if (first_line == null
				...
				printLog("Message received from " + msg.getRemoteAddress()
						+ ":" + msg.getRemotePort() + " (" + msg.getLength()
						+ " bytes): NOT a SIP message: discarded",
						LogLevel.HIGH);
				printLog("message:\r\n" + msg.toString(), LogLevel.LOWER);
				return;
			}
			// else
			printContLog("Message received from " + msg.getRemoteAddress()
					+ ":" + msg.getRemotePort() + " (" + msg.getLength()
					+ " bytes): " + first_line, LogLevel.HIGH);
			printLog("message:\r\n" + msg.toString(), LogLevel.LOWER);
	...
	private void addConnection(ConnectedTransport conn) {
		...
		} else if (connections.size() >= nmax_connections) { // remove the older
																// unused
																// connection
			printLog(
		...
		for (Enumeration e = connections.keys(); e.hasMoreElements();) {
			...
			printLog(
					"conn-id="
							+ id
							+ ": "
							+ ((ConnectedTransport) connections.get(id))
									.toString(),
					LogLevel.LOW);
		}
	}
	...
	// private static int
	// branch_generator=org.zoolu.tools.Random.nextInt();//=10000;
	 ...
	 * cookie.
	 */
	public static String pickBranch() { // return
										// "z9hG4bK"+String.valueOf(branch_generator++);
										// return "z9hG4bK"+(new
										// MD5(String.valueOf(branch_generator++))).asHex().substring(0,5);
		String str = Long.toString(Math.abs(org.zoolu.tools.Random.nextLong()),
				16);
		if (str.length() < 5)
			str += "00000";
	...
	public String pickBranch(Message msg) {
		...
		if (top_via.hasBranch())
			...
			unique_str += Long
					.toString(msg.getCSeqHeader().getSequenceNumber());
	...
	// private static int
	// tag_generator=org.zoolu.tools.Random.nextInt();//=2101968;
	 ...
	 * session.
	 */
	public static String pickTag() { // return String.valueOf(tag_generator++);
										// return (new
										// MD5(String.valueOf(tag_generator++))).asHex().substring(0,8);
		String str = Long.toString(Math.abs(org.zoolu.tools.Random.nextLong()),
				16);
		if (str.length() < 8)
			str += "00000000";
	 ...
	 * <i>request</i>. This tag can be generated for responses in a stateless
	 * manner - in a manner that will generate the same tag for the same request
	 * consistently.
	 */
	public static String pickTag(Message request) { // return
													// String.valueOf(tag_generator++);
													// return (new
													// MD5(request.toString())).asHex().substring(0,8);
		return (new SimpleDigest(8, request.toString())).asHex();
	}

	/** Seed for the call-id gernerator */
	// private static int
	// callid_generator=org.zoolu.tools.Random.nextInt();//=19681002;
	 ...
	 */
	public String pickCallId() { // return
									// String.valueOf(callid_generator++)+"@"+via_addr;
									// return (new
									// MD5(String.valueOf(callid_generator++))).asHex().substring(0,16)+"@"+getViaAddress();
		String str = Long.toString(Math.abs(org.zoolu.tools.Random.nextLong()),
				16);
		if (str.length() < 12)
			str += "000000000000";
	...
	/**
	 * Adds a new log string to the default log output, without return-line
	 * (CRLF)
	 */
	private void printContLog(String str, int level) {
		if (event_log != null) {
			String provider_id = (host_ipaddr == null) ? Integer
					...
					+ host_port;
			event_log.print("SipProvider-" + provider_id + ": " + str, level
