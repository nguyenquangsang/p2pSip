...
public class SessionBuilder {
	...
	private VideoQuality mVideoQuality = VideoQuality.defaultVideoQualiy
			.clone();
	...
	public Session build() throws IOException {
		...
		case AUDIO_AAC:
			...
			if (mContext != null)
				stream.setPreferences(PreferenceManager
	...
	/**
	 * Access to the context is needed for the H264Stream class to store some
	 * stuff in the SharedPreferences. Note that you should pass the Application
	 * context, not the context of an Activity.
	 **/
	 ...
	 * 
	 * @param surfaceHolder
	 *            A SurfaceHolder wrapping a valid surface
	 **/
	 ...
	 * {@link #setDestination(InetAddress)}.
	 */
	...
	/**
	 * Returns the id of the {@link android.hardware.Camera} set with
	 * {@link #setCamera(int)}.
	 */
	...
	/**
	 * Returns the SurfaceHolder set with
	 * {@link #setSurfaceHolder(SurfaceHolder)}.
	 */
	...
	public SessionBuilder clone() {
		return new SessionBuilder().setDestination(mDestination)
				...
				.setVideoQuality(mVideoQuality).setVideoEncoder(mVideoEncoder)
				.setFlashEnabled(mFlash).setCamera(mCamera)
				.setTimeToLive(mTimeToLive).setAudioEncoder(mAudioEncoder)
				.setContext(mContext);
	}
