...
public class GraphicalUA extends JFrame implements UserAgentListener,
	...
	public GraphicalUA(SipProvider sip_provider, UserAgentProfile user_profile) {
		...
		icon_hangup = Archive.getImageIcon(Archive.getJarURL(jar_file,
				HANGUP_GIF));
		...
		// Image
		// image=Archive.getImage(Archive.getJarURL(jar_file,"media/local/ua/intro.gif"));
	...
	private void jbInit() throws Exception {
		...
		if (icon_hangup != null && icon_hangup.getIconWidth() > 0)
			jButton2.setIcon(icon_hangup);
		else
			jButton2.setText("Hungup");
	...
	void comboBoxEditor1_actionPerformed(ActionEvent e) { // if a new URL has
		...
		if (name.indexOf("\"") >= 0 || name.indexOf("<") >= 0) { // try to parse
																	// a
																	// NameAddrress
	...
	public void onUaCallIncoming(UserAgent ua, NameAddress caller,
		...
		} else if (ua.user_profile.accept_time >= 0) // automatically accept the
														// call
		{
			display.setText("ON CALL");
			jComboBox1.setSelectedItem(null);
	...
	public static void main(String[] args) {
		...
		try {
			for (int i = 0; i < args.length; i++) {
				...
				if (args[i].equals("-s") && args.length > (i + 1)) // set
																	// automatic
																	// accept
																	// time
				{
				...
				if (args[i].equals("-g") && args.length > (i + 1)) // registrate
																	// the
																	// contact
																	// url
				{
					...
					if (time.charAt(time.length() - 1) == 'h')
						opt_expires = Integer.parseInt(time.substring(0,
								time.length() - 1)) * 3600;
					else
						opt_expires = Integer.parseInt(time);
					continue;
				}
				...
				if (args[i].equals("-t") && args.length > (i + 1)) // set the
																	// call
																	// duration
				{
				...
				if (args[i].equals("-i") && args.length > (i + 1)) // set the
																	// re-invite
																	// time
				{
				...
				if (args[i].equals("-r") && args.length > (i + 1)) // redirect
																	// the call
																	// to a new
																	// url
				{
				...
				if (args[i].equals("-q") && args.length > (i + 1)) // transfers
																	// the call
																	// to a new
																	// user
																	// (REFER)
				...
				if (args[i].equals("-m") && args.length > (i + 1)) // set the
																	// local
																	// media
																	// port
				{
				...
				if (args[i].equals("--via-addr") && args.length > (i + 1)) // via
																			// addr
				{
					opt_via_addr = args[++i];
					continue;
				}
				if (args[i].equals("-p") && args.length > (i + 1)) // set the
																	// local sip
																	// port
				{
				...
				if (args[i].equals("--from-url") && args.length > (i + 1)) // user's
																			// AOR
				{
					opt_from_url = args[++i];
					continue;
				}
				if (args[i].equals("--contact-url") && args.length > (i + 1)) // user's
																				// contact_url
				{
				...
				if (args[i].equals("--send-file") && args.length > (i + 1)) // send
																			// audio
																			// file
				{
				...
				if (args[i].equals("--recv-file") && args.length > (i + 1)) // receive
																			// audio
																			// file
				{
				...
				if (args[i].equals("--debug-level") && args.length > (i + 1)) // debug
																				// level
				{
					opt_debug_level = Integer.parseInt(args[++i]);
					continue;
				}
				if (args[i].equals("--log-path") && args.length > (i + 1)) // log
																			// path
				{
					opt_log_path = args[++i];
					continue;
				...
				if (!args[i].equals("-h"))
					System.out
							.println("unrecognized param '" + args[i] + "'\n");

				System.out.println("usage:\n   java GraphicalUA [options]");
				System.out.println("   options:");
				System.out.println("   -h              this help");
				System.out
						.println("   -f <file>       specifies a configuration file");
				System.out.println("   -s <secs>       auto answer time");
				System.out
						.println("   -t <secs>       auto hangup time (0 means manual hangup)");
				System.out
						.println("   -g <time>       registers the contact URL with the registrar server");
				System.out
						.println("                   where time is the duration of the registration, and can be");
				System.out
						.println("                   in seconds (default) or hours (-g 7200 is the same as -g 2h)");
				System.out
						.println("   -u              unregisters the contact URL with the registrar server");
				System.out.println("                   (is the same as -g 0)");
				System.out
						.println("   -z              unregisters ALL the contact URLs");
				System.out
						.println("   -i <secs>       re-invite after <secs> seconds");
				System.out
						.println("   -r <url>        redirects the call to new user <url>");
				System.out
						.println("   -q <url> <secs> transfers the call to <url> after <secs> seconds");
				System.out
						.println("   -o              no offer in invite (offer/answer in 2xx/ack)");
				System.out.println("   -a              audio");
				System.out.println("   -v              video");
				System.out
						.println("   -m <port>       (first) local media port");
				System.out
						.println("   -p <port>       local SIP port, used ONLY without -f option");
				System.out
						.println("   --via-addr <addr>   host via address, used ONLY without -f option");
				System.out
						.println("   --from-url <url>    user's address-of-record (AOR)");
				System.out.println("   --contact-url <url> user's contact url");
				System.out
						.println("   --username <addr>   user name used for authentication");
				System.out
						.println("   --real <addr>       realm used for authentication");
				System.out
						.println("   --passwd <addr>     passwd used for authentication");
				System.out
						.println("   --recv-only         receive only mode, no media is sent");
				System.out
						.println("   --send-only         send only mode, no media is received");
				System.out
						.println("   --send-tone         send only mode, an audio test tone is generated");
				System.out
						.println("   --send-file <file>  audio is played from the specified file");
				System.out
						.println("   --recv-file <file>  audio is recorded to the specified file");
				System.out
						.println("   --debug-level <n>   debug level (level=0 means no log)");
				System.out
						.println("   --log-path <path>   base path for all logs (./log is the default value)");
			...
			user_profile.username = "tobeNguyen";
