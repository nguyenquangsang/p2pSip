...
public class UserAgent extends CallListenerAdapter {
	...
	public UserAgent(SipProvider sip_provider, UserAgentProfile user_profile,
		...
		if (user_profile.contact_url == null) {
			user_profile.contact_url = "sip:" + user_profile.username + "@"
					+ sip_provider.getViaAddress();
			if (sip_provider.getPort() != SipStack.default_port)
				user_profile.contact_url += ":" + sip_provider.getPort();
			if (!sip_provider.getDefaultTransport().equals(
					SipProvider.PROTO_UDP))
				user_profile.contact_url += ";transport="
		...
		if (!user_profile.use_rat) {
			try {
				...
				clip_on = new AudioClipPlayer(
						Archive.getAudioInputStream(Archive.getJarURL(jar_file,
								CLIP_ON)), null);
				clip_off = new AudioClipPlayer(
						Archive.getAudioInputStream(Archive.getJarURL(jar_file,
								CLIP_OFF)), null);
				clip_ring = new AudioClipPlayer(
						Archive.getAudioInputStream(Archive.getJarURL(jar_file,
								CLIP_RING)), null);
			} catch (Exception e) {
	...
	protected void launchMediaApplication() {
		...
		if (user_profile.audio && local_audio_port != 0
			...
			} else if (user_profile.use_jmf) { // check if JMF is supported
				try {
					Class myclass = Class.forName("local.ua.JMFAudioLauncher");
					Class[] parameter_types = { Class.forName("int"),
							Class.forName("java.lang.String"),
							Class.forName("int"), Class.forName("int"),
		...
		if (user_profile.video && local_video_port != 0
			...
			if (video_app == null) {
				printLog(
						"No external video application nor JMF has been selected: Video not started",
						LogLevel.HIGH);
				return;
			}
	 ...
	 * call)
	 */
	public void onCallIncoming(Call call, NameAddress caller,
			NameAddress callee, String sdp, Message invite) {
		...
		if (sdp != null) { // Create the new SDP
			...
			SessionDescriptor new_sdp = new SessionDescriptor(
					remote_sdp.getOrigin(), remote_sdp.getSessionName(),
		...
		if (clip_ring != null)
			clip_ring.loop();
		if (ua_listener != null)
			ua_listener.onUaCallIncoming(this, caller, callee);
	}
	...
	public void onCallRinging(Call call, Message resp) {
		...
		if (clip_on != null)
			clip_on.replay();
		if (ua_listener != null)
			ua_listener.onUaCallRinging(this);
	}
	...
	public void onCallAccepted(Call call, String sdp, Message resp) {
		...
		if (clip_on != null)
			clip_on.replay();
		if (ua_listener != null)
			ua_listener.onUaCallAccepted(this);
	...
	public void onCallReInviteRefused(Call call, String reason, Message resp) {
		...
		if (ua_listener != null)
			ua_listener.onUaCallFailed(this);
	}
	...
	public void onCallCanceling(Call call, Message cancel) {
		...
		if (clip_off != null)
			clip_off.replay();
		if (ua_listener != null)
			ua_listener.onUaCallCancelled(this);
	}
	...
	public void onCallClosing(Call call, Message bye) {
		...
		if (clip_off != null)
			clip_off.replay();
		if (ua_listener != null)
			ua_listener.onUaCallClosed(this);
		changeStatus(UA_IDLE);
	}
	...
	public void onCallTransferSuccess(ExtendedCall call, Message notify) {
		...
		call_transfer = null;
		if (ua_listener != null)
			ua_listener.onUaCallTrasferred(this);
	}
	 ...
	 * completed
	 */
	public void onCallTransferFailure(ExtendedCall call, int code,
